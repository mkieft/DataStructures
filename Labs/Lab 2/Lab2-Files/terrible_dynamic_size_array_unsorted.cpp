...
void init(int_array& arr)
{
  ...
  arr.data = new int[arr.capacity];
											///FIX: initialziing and setting to nothing
}
...
bool contains(const int_array& arr, const int& target)
{
	...
	for (i = 0; i < arr.count; ++i)				///FIX: remove ;
	{
		if (arr.data[i] == target) return true;
		///else return false;					///FIX: dont want else case
	}
	return false;
}
...
void add(int_array& arr, const int& payload)
{
	...
	if ((arr.count == arr.capacity))		///Problem 
		resize(arr);
...
bool remove(int_array& arr, const int& target)
{
	...
	///if ((arr.count = 0)) 					///FIX: (arr.count needs "==" 0)
	if ((arr.count == 0 ))
		return false;

...
void selectSort(int arr[], int n)
{
	...
	for (int i=0; i<n-1; i++)	// outer loop tracks the sorted portion of the list -- grow sorted size by one
	{
		...
		{
		 	if (arr[j] < arr[index_min_value]) //min value starts at beginning of sorted looks for value smaller than
                   index_min_value=j;
		}
...
void insertion_sort (int arr[], int length)
{
	...
	{
		j = i;
		while (j > 0 && arr[j] < arr[j-1])	// swaps until in the right spot 
...
void bubbleSort(int arr[], int n)
{
   ...
   for (i = 0; i < n-1; i++)      
   {
       ...
       for (j = 0; j < n-i-1; j++) //goes from beginning to end of unsorted array (n-i-1)
       {
           if (arr[j] > arr[j+1])
           {
              swap(&arr[j], &arr[j+1]);

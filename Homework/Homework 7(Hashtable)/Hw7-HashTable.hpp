...
class HashTable {
  public:
    HashTable(int hashTableSize); //constructor
    ~HashTable();   //destructor

    void addWord(std::string word); //insert word into hash table
    bool isInTable(std::string word); //checks if word is already in
    void incrementCount(std::string word); //adds +1 to count of non stop words
    
    void printTopN(int n); //gets top "n" most common words
    int getNumCollisions(); //counts number of collisions
    int getNumItems(); //number of items in list

...
void Graph::BFTraversalLabel(std::string startingCity, int distID)
{
	...
	{
		vertex *next1 = queue.front();
		queue.pop();

		for(unsigned int i = 0; i < next1->adj.size(); i++)
		{
			if(next1->adj[i].v->visited == false)
			{
				next1->adj[i].v->visited = true;
				next1->adj[i].v->district = distID;
				queue.push(next1->adj[i].v);
			}
		}
	}
}
